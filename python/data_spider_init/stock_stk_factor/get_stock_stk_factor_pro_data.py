...
def update_stock_data(tsCode, startDate, endDate, tusharePro, mysqlConn):
    ...
    # 获取股票列表
    data = pro.stk_factor_pro(ts_code=tsCode, start_date=startDate, end_date=endDate)
    # 转换为DataFrame
    ...
    # 将DataFrame写入数据库
    cursor = cnx.cursor()
    table_name = 'stock_stk_factor_pro_data'
...
def update_stock_curr_data(tsCode, tradeDate, tusharePro, mysqlConn):
    ...
    # 获取股票列表
    data = pro.stk_factor_pro(ts_code=tsCode, trade_date=tradeDate)
    # 转换为DataFrame
    ini_df = pd.DataFrame(data)
    if ini_df.empty:
        print(f"No data for {tsCode}. Skipping...")
...
def update_stock_data_day(tradeDate, tusharePro, mysqlConn):
    ...
    # 获取股票列表
    data = pro.stk_factor_pro(trade_date=tradeDate)
    # 转换为DataFrame
    ini_df = pd.DataFrame(data)
    if ini_df.empty:
...
if __name__ == '__main__':
    ...
    get_curr_data_stock_stk_factor_data_fast("20250101")
