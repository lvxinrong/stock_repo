...
def update_ind_money_flow_ths_by_month(year, month):
    ...
    table_name = 'ind_money_flow_ths_day'
    ...
    try:
        for date in dates:
            ...
            # 从Tushare获取数据
            df = pro.moneyflow_ind_ths(trade_date=date)
...
def update_ind_money_flow_ths_by_trade_date(tradeDate):
    pro = get_tushare_api_pro()
    cnx = get_mysql_conn()
    cursor = cnx.cursor()
    table_name = 'ind_money_flow_ths_day'
    # 从Tushare获取数据
    df = pro.moneyflow_ind_ths(trade_date=tradeDate)
    if df.empty:
...
if __name__ == '__main__':
    update_ind_money_flow_ths_by_month(2025, 1)
    update_ind_money_flow_ths_by_month(2025, 2)
    update_ind_money_flow_ths_by_month(2025, 3)
