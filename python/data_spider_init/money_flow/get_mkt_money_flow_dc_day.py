...
def update_mkt_money_flow_dc_by_month(year, month):
    ...
    table_name = 'mkt_money_flow_dc_day'
    ...
    try:
        for date in dates:
            ...
            # 从Tushare获取数据
            df = pro.moneyflow_mkt_dc(trade_date=date)
            ...
            df = df.rename(columns={'rank': 'rank_value'})
            # 过滤掉不需要插入的字段
...
def update_mkt_money_flow_dc_by_trade_date(tradeDate):
    pro = get_tushare_api_pro()
    cnx = get_mysql_conn()
    cursor = cnx.cursor()
    table_name = 'mkt_money_flow_dc_day'
    # 从Tushare获取数据
    df = pro.moneyflow_mkt_dc(trade_date=tradeDate)
    if df.empty:
...
if __name__ == '__main__':
    update_mkt_money_flow_dc_by_month(2025, 1)
    update_mkt_money_flow_dc_by_month(2025, 2)
    update_mkt_money_flow_dc_by_month(2025, 3)
