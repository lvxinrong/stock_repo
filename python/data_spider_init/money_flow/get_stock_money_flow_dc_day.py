...
def update_stock_money_flow_dc_by_month(year, month):
    ...
    table_name = 'stock_money_flow_dc_day'
...
def update_stock_money_flow_dc_by_trade_date(tradeDate):
    pro = get_tushare_api_pro()
    cnx = get_mysql_conn()
    cursor = cnx.cursor()
    table_name = 'stock_money_flow_dc_day'
    # 从Tushare获取数据
    df = pro.moneyflow_dc(trade_date=tradeDate)
    if df.empty:
...
if __name__ == '__main__':
    update_stock_money_flow_dc_by_month(2025, 1)
    update_stock_money_flow_dc_by_month(2025, 2)
    update_stock_money_flow_dc_by_month(2025, 3)
